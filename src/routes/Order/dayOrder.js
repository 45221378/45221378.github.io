...
export default class Dayorder extends React.Component{
  render() {
    ...
    return (
      <section className="height100 type3">
        {infoList && <div className='orderList orderSearch'>
          {/*<SearchBar placeholder="可输入订单号" cancelText="搜索" onSubmit={searchGoods} onCancel={searchGoods} onChange={setName} showCancelButton/>*/}
          {
                      ...
                      return (
                        <Item className='li' key={idx}>
                          ...
                          <section className='order-name'>
                            ...
                            <p>首次还款日：<span>{item.paymentDate}</span></p>
                            <p>距离最近还款天数：<span>{item.payDays}</span></p>
