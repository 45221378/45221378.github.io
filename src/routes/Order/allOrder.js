...
export default class Allorder extends React.Component{
  render() {
    const {infoList,searchGoods,setName,differBtn,history} = this.props;
    const filterObj = {
                      ...
                      infoList.map((item,idx)=>{
                        return (
                          <Item className='li' key={idx}>
                            ...
                            <header>
                              <div>
                                <span>{item.userName||item.custName}</span><span className="padl6">{item.userPhone||item.phone}</span>
                            ...
                            <section className='order-name'>
                              ...
                              {item.status&&filterObj[item.status].btn2&&<button onClick={()=>differBtn(item.status,item.orderId)}>{filterObj[item.status].btn2}</button>}
                              <button onClick={() => {history.push(`/orderDetail/${item.orderId}`)}}>查看</button>
                            </section>
                          </Item>
                        )
                      })
